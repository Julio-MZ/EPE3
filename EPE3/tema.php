...
<html lang="es">
	...
	<body>
		...
		<div class="container-fluid p-5">
			<?php $result = mysqli_query($con, "SELECT * FROM temas WHERE idCategoria = " .mysqli_real_escape_string($con, $_GET['id']));
			if(!$result){
				echo 'No se pudieron mostrar los temas. Inténtelo de nuevo más tarde.';
			}else{
				if(mysqli_num_rows($result) == 0){
					echo 'Aún no se han definido temas.';
				}else{ ?>
					<table class="table table-striped table-bordered table-hover table-light">
						...
						<tbody>
							<?php while($row = mysqli_fetch_assoc($result)){ ?>
								<tr>
									<td><h4><a href="publicacion.php?id= <?php echo $row['idTema'] ?> "> <?php echo $row['nombre'] ?> </a></h4></td>
