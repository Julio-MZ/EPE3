...
<html lang="es">
	...
	<body>
		...
		<div class="container-fluid p-5">
			<?php $result = mysqli_query($con, "SELECT p.idTema, p.contenido, p.fecha, p.idUsuario, u.idUsuario, u.nombre FROM publicaciones as p LEFT JOIN usuarios as u ON p.idUsuario = u.idUsuario WHERE p.idTema = " .mysqli_real_escape_string($con, $_GET['id']));
			if(!$result){
				echo 'No se pudieron mostrar las publicaciones. Inténtelo de nuevo más tarde.';
			}else{
					...
					<table class="table table-striped table-bordered table-hover table-light">
						...
						<tbody>
							<?php while($row = mysqli_fetch_assoc($result)){ ?>
								<tr>
									<td> <?php echo $row['nombre'] ?> <br><br> <?php echo date('d-m-Y h:m:s', strtotime($row['fecha'])); ?> </td>
